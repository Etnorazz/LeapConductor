...
def trackStartAction(controller, tracknum=2):
    print "Start Track"
    controller.midi_interface.play_track(tracknum)
    controller.stopped = False

def trackStopAction(controller, tracknum=2):
    print "Stop Track"
    controller.midi_interface.stop_track(tracknum)
    controller.stopped = True

def lowerVolumeAction(controller, vol=0.5, tracknum=2):
...
def raiseVolumeAction(controller, vol=0.5, tracknum=2):
    volume = controller.current_vol+ vol
